local function redir_open_win(buf, stderr_p)
...
local function redir_shell_command(cmd, lines, stderr_p)
	local shell_cmd = {
		"sh",
		"-c",
		cmd,
...
local function redir(args)
	...
	cmd = cmd:sub(2)
	redir_shell_command(cmd, lines, stderr_p)
end
